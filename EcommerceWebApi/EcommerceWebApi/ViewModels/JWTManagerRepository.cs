...
namespace EcommerceWebApi.ViewModels
{
    public class JWTManagerRepository : IJWTMangerRepository
    {
        ...
        public Tokens Authenicate(LoginViewModel registerViewModel, bool IsRegister)
        {
            ...
            if (IsRegister)
            {
                if (db.TblLogin.Any(x => x.UserName == registerViewModel.UserName && x.Password == registerViewModel.Password))
                {
                    _isUserExists = true;
            ...
            {
                _isAdmin = db.TblLogin.Any(x => x.UserName == registerViewModel.UserName && x.Password == registerViewModel.Password && x.IsAdmin == 1);
            }
            UserRecords = db.TblLogin.ToList().ToDictionary(x => x.UserName, x => x.Password);
